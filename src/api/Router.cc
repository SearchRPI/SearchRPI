...
void Router::setupRoutes(crow::SimpleApp &app) {
  ...
  CROW_ROUTE(app, "/search")
      .methods(crow::HTTPMethod::GET)([](const crow::request &req) {
        ...
        std::string query =
            query_params.get("query") ? query_params.get("query") : "";
        int start = query_params.get("start")
                        ? std::stoi(query_params.get("start"))
                        : 0;
        int end =
            query_params.get("end") ? std::stoi(query_params.get("end")) : 10;
        ...
        response["docs"] = crow::json::wvalue::list(
            {crow::json::wvalue(
                 {{"id", 1234},
                  ...
                  {"snippet",
                   "Text under link that is relevant for query here"}}),
             crow::json::wvalue(
                 {{"id", 2346},
  ...
  CROW_ROUTE(app, "/favicon/<int>")
      .methods(crow::HTTPMethod::GET)([](const crow::request &req, int docid) {
        crow::response res;
        res.code = 200;
  ...
  CROW_ROUTE(app, "/backend/update")
      .methods(crow::HTTPMethod::PUT)([](const crow::request &req) {
        auto body = crow::json::load(req.body);
        if (!body || !body.has("url") || !body.has("title") || !body.has("raw"))
  ...
  CROW_ROUTE(app, "/backend/update-favicon/<int>")
      .methods(crow::HTTPMethod::PUT)([](const crow::request &req, int docid) {
        // dummy response
        return crow::response(200);
      });

  // Get document ID by URL
  CROW_ROUTE(app, "/backend/getdocid/<string>")
      .methods(crow::HTTPMethod::GET)(
          [](const crow::request &req, std::string url) {
            // dummy response
            crow::json::wvalue response;
            int doc_id = 100;
  ...
  CROW_ROUTE(app, "/backend/pagerank/<int>")
      .methods(crow::HTTPMethod::PUT)([](const crow::request &req, int docid) {
        // dummy response
        crow::json::wvalue response;
        response["status"] = "Success";
